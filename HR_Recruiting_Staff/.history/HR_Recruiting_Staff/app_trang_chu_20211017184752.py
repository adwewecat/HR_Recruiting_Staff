...
def brand(ma_brand):
    if ma_brand == 'HT' or ma_brand == 'HT':
        chuoi_html_chi_tiet_ban_tin = CHUOI_HTML_CHI_TIET_BAN_TIN(ma_brand)
    else:
        chuoi_html_chi_tiet_ban_tin = '<h1>Trang Này Không Tồn Tại, Đừng Tự Gõ Vào Nữa Năn Nỉ Đóa.....</h1>'
        Markup(chuoi_html_chi_tiet_ban_tin)


    if request.form.get('user_dang_nhap'):
        print(request.form.get('user_dang_nhap'))
    return render_template('chi_tiet_ban_tin.html',CHUOI_HTML_HIEN_THI_CHI_TIET_BAN_TIN = chuoi_html_chi_tiet_ban_tin )
