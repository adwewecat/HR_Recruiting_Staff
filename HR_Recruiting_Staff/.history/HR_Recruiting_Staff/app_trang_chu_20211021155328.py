...
def brand(ma_brand):
    ...
    return render_template('chi_tiet_ban_tin.html',CHUOI_HTML_HIEN_THI_CHI_TIET_BAN_TIN = chuoi_html_chi_tiet_ban_tin,IMG=img , DANG_NHAP_THAT_BAI = Markup(chuoi))
...
def nha_hang():
    if session.get('session_id_nha_hang') is not None:
        ...
        if phancap == 'nhahang':
            ...
            return render_template('Nha_Hang/nha_hang.html' , HIEN_THI_NHA_HANG_TIN_CON_SU_DUNG = chuoi_html_ban_tin_con_su_dung , HIEN_THI_HTML_THONG_TIN_NHA_HANG = chuoi_html_nha_hang_thong_tin,HIEN_THI_HTML_THONG_TIN_NHA_HANG_HOAN_THANH = chuoi_html_ban_tin_hoan_thanh)
    return redirect(url_for('dang_xuat'))
