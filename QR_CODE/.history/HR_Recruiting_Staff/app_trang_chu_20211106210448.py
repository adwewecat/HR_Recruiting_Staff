...
from QR_CODE.thu_vien.xu_ly_form import *
from QR_CODE.thu_vien.xu_ly_3L import *
# from GGG.thu_vien.connection import *
import socket  
from QR_CODE.thu_vien.connection import *
import datetime
...
API_KEY = 'AIzaSyAphEsUeX1_POM6XX15LP9K69FUOQ92HP4'
map_client = googlemaps.client(API_KEY)
work_place_address= '1 Market st, San Francisco, CA'
           map_client.geocode (work_place_address) response = %3!
pprint(response)
print(response[0])
...
def index():
    ...
    # img.save("/QR_CODE/static/images/conmeo.png")
    chuoidangnhap = ''
...
def nha_hang():
    ...
    if request.form.get('nhahang_dangtin_soluong'):
        ...
        them_ban_tin = INSERT_BAN_TIN_NHA_HANG(session['session_id_nha_hang'],session['session_brand_id'],session['session_district_id'],session['session_region_id'],nhahang_dangtin_note_vitri,nhahang_dangtin_gio_from,nhahang_dangtin_gio_to,nhahang_dangtin_soluong,session['session_username'])

        chuoi_html_ban_tin_con_su_dung  = CHUOI_HTML_NHA_HANG_BAN_TIN_CON_SU_DUNG(session['session_id_nha_hang'])
        return render_template('Nha_Hang/nha_hang.html' , HIEN_THI_NHA_HANG_TIN_CON_SU_DUNG = chuoi_html_ban_tin_con_su_dung , HIEN_THI_HTML_THONG_TIN_NHA_HANG = session['session_chuoi_html_nha_hang_thong_tin'],HIEN_THI_HTML_THONG_TIN_NHA_HANG_HOAN_THANH = session['session_chuoi_html_ban_tin_hoan_thanh'],THONG_TIN_DANG_XUAT = session['session_chuoi_dang_xuat'], DANG_TIN_THANH_CONG = them_ban_tin)    



    if request.form.get('soluong_thuc_te'):
        soluong_thuc_te = request.form.get('soluong_thuc_te')
        id_bai_dang_hoan_thanh =request.form.get('id_bai_dang_hoan_thanh')
        s1 = datetime.datetime.now()
        thoi_gian_hien_tai = s1.strftime("%Y-%m-%d %H:%M:%S")
        update_ban_tin = UPDATE_BAN_TIN_NHA_HANG(id_bai_dang_hoan_thanh,soluong_thuc_te,thoi_gian_hien_tai)

        chuoi_html_ban_tin_con_su_dung  = CHUOI_HTML_NHA_HANG_BAN_TIN_CON_SU_DUNG(session['session_id_nha_hang'])
        chuoi_html_ban_tin_hoan_thanh  = CHUOI_HTML_NHA_HANG_BAN_TIN_HOAN_THANH(session['session_id_nha_hang'])

        return render_template('Nha_Hang/nha_hang.html' , HIEN_THI_NHA_HANG_TIN_CON_SU_DUNG = chuoi_html_ban_tin_con_su_dung , HIEN_THI_HTML_THONG_TIN_NHA_HANG = session['session_chuoi_html_nha_hang_thong_tin'],HIEN_THI_HTML_THONG_TIN_NHA_HANG_HOAN_THANH = chuoi_html_ban_tin_hoan_thanh,THONG_TIN_DANG_XUAT = session['session_chuoi_dang_xuat'], DANG_TIN_THANH_CONG = update_ban_tin) 
...
def ok():
    chuoi_html_nha_hang_thong_tin = CHUOI_HTML_NHA_HANG_THONG_TIN(1)  

    return chuoi_html_nha_hang_thong_tin




@app.route("/onPostt_API", methods = ["POST","GET"])
def onPostt_API():
    a=''
    if request.form.get('gia_tri'):
        a = request.form.get('gia_tri')
